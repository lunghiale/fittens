        ...
        subroutine Rot_Cart(psi,psiv,rot)
        implicit none
        double precision       :: psi,psiv(3),rot(3,3)
       
          rot(1,1)=psiv(1)*psiv(1)*(1-dcos(psi))+dcos(psi)
          rot(1,2)=psiv(1)*psiv(2)*(1-dcos(psi))-psiv(3)*dsin(psi)
          rot(1,3)=psiv(1)*psiv(3)*(1-dcos(psi))+psiv(2)*dsin(psi)
          rot(2,1)=psiv(2)*psiv(1)*(1-dcos(psi))+psiv(3)*dsin(psi)
          rot(2,2)=psiv(2)*psiv(2)*(1-dcos(psi))+dcos(psi)
          rot(2,3)=psiv(2)*psiv(3)*(1-dcos(psi))-psiv(1)*dsin(psi)
          rot(3,1)=psiv(3)*psiv(1)*(1-dcos(psi))-psiv(2)*dsin(psi)
          rot(3,2)=psiv(3)*psiv(2)*(1-dcos(psi))+psiv(1)*dsin(psi)
          rot(3,3)=psiv(3)*psiv(3)*(1-dcos(psi))+dcos(psi)

        return
        end subroutine Rot_Cart
